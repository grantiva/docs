...
do {
    ...
    // riskCategory is available on every plan
    print("Risk category: \(result.deviceIntelligence.riskCategory)")
    // riskScore is nil on the Free tier and in API key mode
...
The returned JWT contains device intelligence and any custom claims you've configured in the dashboard. The SDK caches it in the Keychain, so calling `validateAttestation()` again returns the cached token until it is close to expiry.
