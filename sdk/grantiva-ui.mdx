...
<Note>
GrantivaUI currently covers **feedback and support only**. There are no attestation or feature flag components — call [`validateAttestation()`](/sdk/attestation) and the [flags API](/sdk/feature-flags) directly and build those surfaces yourself.
</Note>
...
2. Enter `https://github.com/grantiva/ios-ui`
...
```swift
dependencies: [
    .package(url: "https://github.com/grantiva/ios-ui.git", from: "1.0.3")
]
```

GrantivaUI depends on `grantiva/ios-sdk` (2.0.1 or later) itself, so you do not need to declare the SDK separately. Add the product with `.product(name: "GrantivaUI", package: "ios-ui")`.
