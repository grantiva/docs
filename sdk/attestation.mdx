...
The SDK automatically caches valid tokens. Calling `attest()` multiple times reuses the cached token until it expires, then performs a fresh attestation.

```swift
// First call — full attestation flow
let result1 = try await grantiva.attest()

// Second call — returns cached token instantly
let result2 = try await grantiva.attest()
```
...
Returns `nil` if no token has been stored yet (call `attest()` first).
