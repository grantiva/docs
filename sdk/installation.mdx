...
In API key mode, the SDK skips real App Attest entirely and returns a synthetic `AttestationResult`: `token` is the placeholder string `"simulator-dev-token"`, `riskScore` is `nil`, and `deviceIntegrity` is `"api_key_mode"`. Requests to the flag and feedback endpoints authenticate with the API key instead of a JWT. Do not ship API keys in production builds.

Calling `validateAttestation()` in the Simulator *without* an API key throws `GrantivaError.simulatorAPIKeyRequired`.
...
The compiled-in version string is available at runtime:

```swift
GrantivaVersion.current  // "2.1.0"
```

It is also sent automatically as part of the device context attached to feedback and flag requests.
