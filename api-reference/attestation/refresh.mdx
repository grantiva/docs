...
| `customClaims` | `object` | The same custom claims projected into the new token, flattened to strings — identical in shape to the `customClaims` on [validate](/api-reference/attestation/validate). Empty object when none are configured |
