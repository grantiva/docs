...
| Status | When |
|--------|------|
| 200 | Event processed — **or** permanently unprocessable (unknown bundle ID, ingestion disabled, missing `appAccountToken`, unmapped product, replayed notification, bad signature) |
| 5xx | Transient server error — Apple will retry |

Permanent conditions intentionally return 200 so Apple stops retrying them. Replay protection is keyed on Apple's `notificationUUID`. A request with no body, or a body that is not a `signedPayload` envelope, returns `400`.
