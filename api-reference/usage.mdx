...
`GET /api/v1/org/usage` returns the same canonical shape and additionally accepts an API key with the `org:read` scope, so scripts and the CLI can read usage. See [Console usage](#console-usage).
...
The same response as `GET /api/v1/tenants/me/usage`, reachable with an API key as well as a session.
...
curl -H "Authorization: Bearer aat_your_api_key" \
  https://api.grantiva.io/api/v1/org/usage
```
