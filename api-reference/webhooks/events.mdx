...
Respond with a 2xx status. Non-2xx responses and transport failures are retried up to 3 attempts total, after 1, 5, and 30 minutes.

<Note>Webhook endpoints can only be created on paid plans (Pro, Business, Enterprise) — see [Webhook Endpoints](/api-reference/webhooks/endpoints).</Note>
...
When the change is a per-environment toggle, `data` also carries the environment slug:
