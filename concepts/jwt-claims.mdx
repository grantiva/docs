...
Your configured claims are nested under `custom_claims`, preserving their JSON types. Grantiva's managed `subscription` object also lives here, on **every** plan — it is injected independently of your custom-claims configuration, so a Free-plan token can carry `custom_claims.subscription` even though Free allows only one custom claim of its own. See [Custom Claims](/concepts/custom-claims) and [Subscription Claims](/concepts/subscription-claims).
